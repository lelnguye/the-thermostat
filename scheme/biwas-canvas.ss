...
(define (rectangle x y width height . colors)
    (let ((rect (list x y width height))
          (stroke "#000000")
          (fill "#FFFFFF"))
        (begin
            (if (and (>= (length colors) 2))
                (begin (set! stroke (car colors)) (set! fill (cadr colors)))
                (if (and (= (length colors) 1))
                    (begin (set! stroke (car colors)) (set! fill (car colors)))))
            (list rect 1 stroke fill))))
...
(define (set-color context style color)
    (if (list? color)
        (let ((r (list-ref color 0)) 
              (g (list-ref color 1))
              (b (list-ref color 2)))
            (js-set! context style (format "rgb(~a, ~a, ~a)" r g b)))
        (js-set! context style color)))
